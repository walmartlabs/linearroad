...
void walkup(int *arr, int start_i)
{
    ...
    { 
        ...
        //    printf("walkup (gp with i):arr[%d], arr[%d], %d, %d\n", start_i/4, start_i, arr[start_i/4], arr[start_i]); // DB
            ...
            // 0)
        // Or you can walk the list to find min-max (log n / 2) 
        int leftmost = start_i / 2 + ((int)floor(sqrt(start_i))/2);
        printf("start_i: %d\n", start_i);
        printf("start_i / 2: %d\n", start_i / 2);
        printf("pre-leftmost: %d\n", (int)floor(sqrt(start_i)));
        printf("leftmost: %d\n", leftmost);
        swap(arr, start_i, leftmost); 
        printf("newest with min-max: walkup:arr[%d], arr[%d], %d, %d\n", leftmost, start_i, arr[leftmost], arr[start_i]); // DB
        // Walk up the leftmost.
        walkup(arr, leftmost);
        //}
    }
    // If right(odd index) is less than parent simply need to walk up.
    else if (start_i % 2 == 1 && arr[start_i] < arr[start_i / 2])
    {
        swap(arr, start_i, start_i / 2); 
        walkup(arr, start_i / 2);
    } 
